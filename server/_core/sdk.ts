...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    // ── Cloudflare Worker cron bypass ──────────────────────────────
    if ((req as any)._isCronInternal === true) {
      return buildInternalCronUser();
    }
...
function buildInternalCronUser(): AuthenticatedUser {
  const now = new Date();
  return {
    id: -1,
    openId: "cron_internal_worker",
