...
async function handleNoindexPosts(_env: CloudflareFunctionEnv) {
  try {
    ...
    if (!res.ok) return { posts: [], noindexPosts: [], noindexCount: 0, available: false };
    const posts = await res.json() as { id: number; title: { rendered: string }; link: string }[];
    const results: any[] = [];
    const batch = posts.slice(0, 8);
    await Promise.all(batch.map(async (post) => {
      try {
        const htmlRes = await fetch(post.link, { signal: AbortSignal.timeout(6000), headers: { "User-Agent": "NCR-Watchdog/1.0" } });
    ...
    return { posts: results, noindexPosts, noindexCount: noindexPosts.length, available: true, checkedAt: new Date().toISOString() };
  } catch (err) {
